...
class ParquetWriter {
   ...
   */
  static async openS3File(schema, s3Client, bucket, object, opts) {
    let outputStream = await parquet_util.s3osopen(s3Client, bucket, object, opts);

    opts = Object.assign({}, opts, { isS3File: true });
  ...
  async appendRow(row) {
    ...
    if (this.rowBuffer.rowCount >= this.rowGroupSize) {
      encodePages(this.schema, this.rowBuffer, { useDataPageV2: this.envelopeWriter.useDataPageV2});
      this.envelopeWriter.writeRowGroup(this.rowBuffer);
      this.rowBuffer = {};
    }
  }
...
class ParquetEnvelopeWriter {
   ...
   */
  static async openStream(schema, outputStream, opts) {
    let writeFn = opts && opts.isS3File
      ? parquet_util.s3oswrite.bind(undefined, outputStream)
      : parquet_util.oswrite.bind(undefined, outputStream);
    let closeFn = opts && opts.isS3File
      ? parquet_util.s3osclose.bind(undefined, outputStream)
      : parquet_util.osend.bind(undefined, outputStream);
  ...
  for (let field of schema.fieldList) {
    try {
      ...
      if (opts.useDataPageV2) {
        // console.log('encodePages: field', field)
      ...
      console.log('encodePages error with field', field);
      throw (err);
    }
  }
...
function encodeDataPageV2(column, rowCount, values, rlevels, dlevels, statistics) {
  ...
  let rLevelsBuf = Buffer.alloc(0);
  if (column.rLevelMax > 0) {
    const width = parquet_util.getBitWidth(column.rLevelMax)
    console.log('encodeDataPageV2', {
      PARQUET_RDLVL_TYPE,
      PARQUET_RDLVL_ENCODING,
      rlevels: {
        bitWidth: width,
