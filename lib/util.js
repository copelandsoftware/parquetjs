...
// Assumes s3 output stream.  Could be enhanced to check the T/F response of
// write to see if we need to wait for the drain event.  Ignoring the potential
// for a drain event is a potential security vulnerability.
exports.s3oswrite = function(os, buf) {
  return new Promise((resolve, reject) => {
    os.write(buf, null, (err) => err ? reject(err) : resolve());
  });
};

exports.s3osclose = function(os) {
  return new Promise((resolve, reject) => {
    os.end(null, null, (err) => err ? reject(err) : resolve());
  });
};

// This assumes your environment (either local creds or IAM permission)
// grants you adequate privileges to write to the provided bucket.  This
// could definitely be enhanced.
exports.s3osopen = function(bucket, path, opts) {
  return new Promise((resolve, reject) => {
    const s3Connection = new s3fs(bucket, {});
    const outputStream = s3Connection.createWriteStream(path, {});

    outputStream.on('error', (err) => {
      console.log('error from output stream', err);
      reject(err);
    });

    // There is no 'open' event here, so just resolve the stream
    resolve(outputStream);
  });
};
